...
class TPSCommand : Command("", "tps") {
    override fun execute(commandExecutor: CommandExecutor, arguments: Array<String?>) {
        ...
        commandExecutor.msg(
            (ChatColor.WHITE.toString() + "CPU Architechure: " + ChatColor.AQUA.toString()
                    + getCpuArch(osBean))
        )
        commandExecutor.msg(
    ...
    fun getCpuUsage(osBean: OperatingSystemMXBean): Double {
        // Get the OperatingSystemMXBean instance

        // Get the system CPU load

        @Suppress("deprecation") val cpuLoad = osBean.systemCpuLoad * 100

        return round(cpuLoad)
    }

    fun getCpuArch(osBean: OperatingSystemMXBean): String {
        // Get the OperatingSystemMXBean instance

        return osBean.arch
    }

    fun getOSName(osBean: OperatingSystemMXBean): String {
        return osBean.name
    }

    fun getTpsStr(tps: Double): String {
        val color = color(tps)
    ...
    companion object {
        ...
        private fun color(tps: Double): ChatColor {
            return (if (tps > 18.0) ChatColor.GREEN else if (tps > 16.0) ChatColor.YELLOW else ChatColor.RED)
        }
    }
}
