...
class EntityMetadataImpl(private val entityClass: Class<out EntityImpl?>) : EntityMetadata {
    ...
    fun set(index: EntityMetadataIndex, value: Any) {
        ...
        require(index.type.dataType.isAssignableFrom(value.javaClass)) { "Cannot assign " + value + " to " + index + ", expects " + index.type }

        require(index.appliesTo(entityClass)) {
            ("Index " + index + " does not apply to " + entityClass.simpleName
                    + ", only " + index.appliesTo.simpleName)
        }

        val prev = map.put(index, value)
        if (prev != value) changes.add(EntityMetadata.Entry(index, value))
    }
