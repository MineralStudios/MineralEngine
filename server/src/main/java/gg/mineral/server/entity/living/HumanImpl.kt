...
open class HumanImpl(id: Int, world: WorldImpl) : LivingImpl(id, world), Human {
    ...
    @Getter
    protected val visibleEntities: Int2ObjectOpenHashMap<IntArray> = object : Int2ObjectOpenHashMap<IntArray?>() {
    ...
    @Getter
    @Setter
    var extraKnockback: Boolean = false

    @Getter
    @Setter
    protected var gamemode: Gamemode = Gamemode.SURVIVAL

    @Getter
    @Setter
    private val sprinting = false
    ...
    override fun swingArm() {
        for (id in visibleEntities.keys) {
    ...
    fun updateArm(human: HumanImpl) {
        if (!human.swingingArm || human.swingingTicks >= human.swingSpeed() / 2 || human.swingingTicks < 0) {
            ...
            if (this is PlayerImpl) player.getConnection().queuePacket(AnimationPacket(human.id, 0.toShort()))
        }
    }
