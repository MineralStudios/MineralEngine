...
open class ChunkImpl(
    @field:Getter private val world: World,
    /**
     * The coordinates of this chunk.
     */
    @field:Getter private val x: Byte, @field:Getter private val z: Byte
) : Chunk {
    @Getter
    private val entities: IntSet = IntOpenHashSet()
     ...
     */
    private var biomes: ByteArray?

    fun resetCache() {
        for (i in cache.indices) cache[i] = null
    }

    fun getCache(skylight: Boolean, compress: Boolean): ChunkDataPacket? {
    ...
    fun setCache(skylight: Boolean, compress: Boolean, packet: ChunkDataPacket?) {
        val index = if (skylight && compress) 3
        else if (skylight) 2
        else if (compress) 1
        else 0
    ...
    override fun getType(x: Int, z: Int, y: Short): Int {
        val section = getSection(y.toInt())
        return if (section == null) 0 else (section.types[section.index(x, y.toInt(), z)].toInt() and 0xff)
    }
    ...
    fun setType(x: Int, z: Int, y: Short, type: Int) {
        ...
        var section = getSection(y.toInt())
        if (section == null) {
            if (type == 0) {
        ...
        val index = section.index(x, y.toInt(), z)
        if (type == 0) {
        ...
        section.types[section.index(x, y.toInt(), z)] = type.toByte()
    ...
    override fun getMetaData(x: Int, z: Int, y: Short): Byte {
        val section = getSection(y.toInt())
        return section?.metaData?.get(section.index(x, y.toInt(), z)) ?: 0
    }

    fun setMetaData(x: Int, z: Int, y: Int, metaData: Byte) {
    ...
    fun getSkyLight(x: Int, z: Int, y: Int): Byte {
        val section = getSection(y)
        return section?.skyLight?.get(section.index(x, y, z)) ?: 0
    }

    fun setSkyLight(x: Int, z: Int, y: Int, skyLight: Int) {
        val section = getSection(y) ?: return
    ...
    fun getBlockLight(x: Int, z: Int, y: Int): Byte {
        val section = getSection(y)
        return section?.blockLight?.get(section.index(x, y, z)) ?: 0
    }

    fun setBlockLight(x: Int, z: Int, y: Int, blockLight: Int) {
        val section = getSection(y) ?: return
        // can't set light on an empty section
    ...
    fun setBiomes(newBiomes: ByteArray) {
        ...
        System.arraycopy(newBiomes, 0, biomes, 0, biomes!!.size)
    }
    ...
    open fun toPacket(skylight: Boolean, compress: Boolean): ChunkDataPacket {
        ...
        for (i in 0..15) {
            ...
            for (j in section.types.indices) {
                val type = section.types[j].toInt() and 0xFF
                if (type > 0xFF) {
                    hasAddData = true
                    break
                }
            ...
            if (hasAddData) {
                addBitmap = addBitmap or (1 shl i)
                numAddSections++
            }
        }
        ...
        val sizeAddData = numAddSections * 2048
        val sizeBiomes = if (entireChunk) 256 else 0

        val totalSize = sizeBlockTypes + sizeMetadata + sizeBlockLight + sizeSkyLight + sizeAddData + sizeBiomes
        ...
        val posBiomes = posAddData + sizeAddData
        ...
        if (addBitmap != 0) {
            sectionIndex = 0
            for (i in 0..15) {
                if ((addBitmap and (1 shl i)) == 0) continue

                val section = sections[i] ?: continue

                val offset = posAddData + sectionIndex * 2048
                System.arraycopy(section.addData.rawData, 0, output, offset, 2048)
        ...
        if (entireChunk) {
            if (biomes == null) biomes = ByteArray(256)
