 ...
 */
     ...
     * The value.
     */
    private val value: MutableMap<String, Tag<*>> = Object2ObjectLinkedOpenHashMap()

    override fun getValue(): Map<String?, Tag<*>?> {
        return value
    }

    override fun valueToString(bldr: StringBuilder) {
        bldr.append(value.size).append(" entries\n{\n")
        for ((key, value1): Map.Entry<String, Tag<*>> in value) bldr.append("    ").append(
        key
        ).append(": ")
    ...
    fun getBool(key: String): Boolean {
        return get<Byte, ByteTag>(key, ByteTag::class.java)!!.toInt() != 0
    }

    fun getByte(key: String): Byte {
        val tag = get<Byte, ByteTag>(key, ByteTag::class.java)
        return tag ?: 0
    }
    ...
    fun getByteArray(key: String): ByteArray {
        return get<ByteArray, ByteArrayTag>(key, ByteArrayTag::class.java)!!
    }

    fun getString(key: String): String? {
        return get(key, StringTag::class.java)
    }

    fun getIntArray(key: String): IntArray {
        return get(key, IntArrayTag::class.java)!!
    }
    ...
    fun <V> getList(key: String, type: TagType): List<V?> {
        ...
        result.addAll(
            original.stream().map { item: Tag<*>? -> if (item == null) null else item.value as V }
                .collect(Collectors.toList()))
        return result
    }

    fun getCompound(key: String): CompoundTag {
        return getTag(key, CompoundTag::class.java)
    }

    fun getCompoundList(key: String): List<CompoundTag> {
        return getTagList(key, TagType.COMPOUND) as List<CompoundTag>
    }

    /**///////////////////////////////////////////////////////////////////////// */ // Simple is
    fun isByte(key: String): Boolean {
        return `is`(key, ByteTag::class.java)
    }

    fun isShort(key: String): Boolean {
        return `is`(key, ShortTag::class.java)
    }

    fun isInt(key: String): Boolean {
        return `is`(key, IntTag::class.java)
    }

    fun isLong(key: String): Boolean {
        return `is`(key, LongTag::class.java)
    }

    fun isFloat(key: String): Boolean {
        return `is`(key, FloatTag::class.java)
    }

    fun isDouble(key: String): Boolean {
        return `is`(key, DoubleTag::class.java)
    }

    fun isByteArray(key: String): Boolean {
        return `is`(key, ByteArrayTag::class.java)
    }

    fun isString(key: String): Boolean {
        return `is`(key, StringTag::class.java)
    }

    fun isIntArray(key: String): Boolean {
        return `is`(key, IntArrayTag::class.java)
    }
    ...
    fun isCompound(key: String): Boolean {
        return `is`(key, CompoundTag::class.java)
    }
    ...
    /**///////////////////////////////////////////////////////////////////////// */ // Fancy sets
    fun <V> putList(key: String, type: TagType, value: List<V>) {
        val result: MutableList<Tag<*>> = ArrayList(value.size)
        for (item in value) result.add(type.newObj(item))
