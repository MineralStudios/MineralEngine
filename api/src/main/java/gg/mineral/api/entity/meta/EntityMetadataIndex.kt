...
@Getter
enum class EntityMetadataIndex {
    ...
    private val index = 0
    private val type: EntityMetadataType? = null
    private val appliesTo: Class<out Entity?>? = null

    fun appliesTo(clazz: Class<out Entity?>): Boolean {
    ...
    companion object {
        fun getIndex(index: Int, type: EntityMetadataType): EntityMetadataIndex? {
            ...
            for (entry in entries) if (entry.getIndex() == index && entry.getType() == type) {
                output = entry
                break
            }
